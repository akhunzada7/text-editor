...
main_application.title('vpad txt editor')
...
main_menu.add_cascade(label='color_theme',menu=color_them)
######################## toolbar ######################
...
def change_uderline():
    ...
    if text_property.actual()['underline']==1:
        text_editor.configure(font=(current_font_family,current_font_size,'normal'))
underline_btn.configure(command=change_uderline)
...
def changed(event=None):
    ...
    if text_editor.edit_modified():
        ...
        status_bar.configure(text=f'character : {characters} words : {words}')
...
def exit_func(event=None):
    ...
    try:
        if text_changed:
            ...
            if mbox is True:
                if url:
                    content=text_editor.get(0.1,tk.END)
                    with open(url,'w',encoding='utf-8') as fw:
                        fw.write(content)
                        main_application.destroy()
                else:
                    content2=str(text_editor.get(0.1,tk.END))
                    url = filedialog.asksaveasfile(mode='w',defaultextension='.txt',filetypes=(('Text file','*.txt'),('All files','*.*')))
                    url.write(content2)
                    url.close()
                    main_application.destroy()
...
def find_func(event=None):
    ...
    #fram
    find_fram=ttk.LabelFrame(find_dialogue,text='Find/Replace')
    find_fram.pack(pady=20)
    #lable
    text_find_lable=ttk.Label(find_fram,text='find: ')
    text_replace_lable=ttk.Label(find_fram,text='Replace')

    #entry
    find_input=ttk.Entry(find_fram,width=30)
    replace_input=ttk.Entry(find_fram,width=30)
    ##buton
    find_button=ttk.Button(find_fram,text='find')
...
count=0
for i in color_dict:
    color_them.add_radiobutton(label=i,image=color_icon[count],variable=theme_choice,compound=tk.LEFT,command=change_theme)
    count += 1

#-----------------------end main menu functionaly--------------------
